 ...
 * Stefan Bylund 2018
 *
 * Implementation of layer2.h; a limited C API for using the layer 2 screen of
 * the Sinclair ZX Spectrum Next for displaying images by the Magnetic Scrolls
...
void layer2_set_clip_window(uint8_t x, uint8_t y, uint16_t width, uint8_t height)
{
...
void layer2_clear_screen(layer2_screen_t screen, uint8_t color)
{
    ...
    for (uint8_t page = screen_base_page; page < screen_base_page + 6; ++page)
    {
        ZXN_WRITE_MMU2(page);
                        ...
                        uint8_t *buf_256)
{
    ...
    // Load screen in max 8 KB chunks using MMU slot 2 at address 0x4000.

    page = GET_SCREEN_BASE_PAGE(screen);
    size = height << 8;
    ...
    while (size > 0)
    {
        ...
        esx_f_read(filehandle, SCREEN_ADDRESS, buffer_size);
        if (errno)
        {
            goto end;
        }
