...
;; as ENTER. The Magnetic interpreter needs to use CAPS+7 and CAPS+6 (up and
;; down arrow keys) to scroll the graphics image up and down so the ASCII codes
;; returned for CAPS+6 and CAPS+7 are modified here. CAPS+SPACE is mapped to an
;; unused ASCII code representing the BREAK key. In addition, CTRL+C and CTRL+D
;; are disabled so that the user cannot generate interrupts or close the input
;; stream.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
in_key_translation_table:
   ...
   defb 'P','O','I','U','Y'      ; P, O, I, U, Y
   defb 13,'L','K','J','H'       ; ENTER, L, K, J, H
   defb ASCII_CODE_BREAK,255,'M','N','B'  ; SPACE, SYM SHIFT, M, N, B
