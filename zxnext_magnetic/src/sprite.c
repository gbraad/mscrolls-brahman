 ...
 * Stefan Bylund 2018
 *
 * Implementation of sprite.h; a limited C API for using the hardware sprites of
 * the Sinclair ZX Spectrum Next for displaying the status bar, a scroll prompt
 * and a mouse pointer by the Magnetic Scrolls interpreter interface.
 ******************************************************************************/
...
#define X_LSB(x) ((uint8_t) ((x) & 0x00FF))
#define X_MSB(x) ((uint8_t) (((x) & 0x0100) >> 8))
