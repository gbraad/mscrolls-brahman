...
int32_t NL_CheckSyntax(char *line_buf, int32_t id, int32_t nr_of_types, int32_t *types, int32_t type_index,
    ...
    case VERB:
      ...
      if (result == OK) {
        ...
        if (state == 54 || state == 56) {
          ...
          if (parsed_input->action1 != NO_ID) {
            /* Yes, it was a helper */
            parsed_input->action2 = id;
          }
          else {
            parsed_input->action1 = id;
          }
    ...
    case DIRECTIONS:
      ...
      if (result == OK) {
        ...
        if (old_state == 1 || old_state == 18)
          ...
          parsed_input->action1 = LookUpId("ga");
