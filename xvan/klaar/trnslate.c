 ...
 /* index tells which subject to use from parsed_input */
{
  ...
  if (FilledOut(&(parsed_input->actor))) {
    switch(Find(&(parsed_input->actor), parsed_input->scope, line_buf, &actor_hits)) {    switch(Find(&(parsed_input->actor), NO_ID, parsed_input->scope, line_buf, &actor_hits)) {  /* @!@ */      case ERROR:
        result.tag = ERROR;
