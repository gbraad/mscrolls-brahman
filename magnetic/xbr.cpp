...
static uint32_t pixel_diff1(uint32_t x, uint32_t y, const uint32_t *r2y)
{
...
#define VMASK 0x0000ff
    ...
    return (uabs((x >> 24) - (y >> 24))) +
           (uabs((yuv1 & YMASK) - (yuv2 & YMASK)) >> 16) +
           (uabs((yuv1 & UMASK) - (yuv2 & UMASK)) >>  8) +
           uabs((yuv1 & VMASK) - (yuv2 & VMASK));
}
#endif
...
static uint32_t pixel_diff(uint32_t x, uint32_t y)
{
    ...
    int y1 = 66*r1 + 129*g1 + 25*b1;
    int u1 = -38*r1 - 74*g1 + 112*b1;
    int v1 = 112*r1 - 94*g1 - 18*b1;

    // NB: no alpha
    uint32_t d1 = (abs(y1) + abs(u1) + abs(v1)) >> 8;
    return d1;
}
