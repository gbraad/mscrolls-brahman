...
char *fname;
{
	...
	putc((m-1)&0xFF,fout);	/* put 6502 style size on UNHUFFED at start */
	putc(((m-1)>>8)&0xFF,fout);
	/* output tree, 16 bytes of left, 16 bytes of right */
	for (j = 0; j < 16; j++)
