...
`Strands` models the location of objects in terms of containers.
...
* _Some scenery objects_
  Let's say you have a river that flows through various locations. The river, the water and all things connected to it are in _all those locations_. However, in `Strands` you most likely wouldn't do this as multiple locations, but instead create a parent of the locations in question, say `riverplaces`, which would host the behviour of the river.
...
Simple animation can be made as animated PNGs, but complex ones are built using [Spine](http://esotericsoftware.com/). The Strands GUI has the spine 2D animation runtime built in and all that's needed is to drop the spine output files into the `images` folder and use them.
