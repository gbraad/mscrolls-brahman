...
int32_t Entrance(int32_t);
int32_t Contents(int32_t);
 ...
 int32_t  **trigger;
{
  ...
  /* is included in the IsLocId() and IsObjId() functions.      */
    ...
    sprintf(outputline, "GetPar(): unknown parameter syntax: %d.\n", next);
    Output(outputline);
    return(ERROR);
  }
  return(OK);
 ...
 int32_t **trigger;
{
  ...
  if (IsIntAct(next)) {
    switch (next) {
      case OWNER:
        next = XeqIntAct(next, trigger);
        /* error in return value are caught by the last else statement of this function. */
        break;
      default:
        sprintf(outputline, "GetLvaluePar(): used function does not return location or object id.\n");
        Output(outputline);
        return(ERROR);
    }
  }
        ...
        *type  = ATTRIBUTES;
        ...
        if (IsTimerId(next)) {
          ...
          *type  = TIM_ID;
          ...
          sprintf(outputline, "GetLvaluePar() (%d): attribute or timer expected.\n", next);
          Output(outputline);
          return(ERROR);
        } /* else */
      } /* else */
 ...
 int32_t  *attribute_index;
{
  ...
  if (IsCAttrId(id)) {
    ...
    *attribute_index = id-FIRST_LOCAL_ATTR_ID;
    ...
    /* error */
    sprintf(outputline, "GetAttributeInfo(): error, %d is not an attribute id.\n", id);
    Output(outputline);
    return(ERROR);
  }
  return(OK);
 ...
 int32_t *index;
{
  ...
  if (*index == nr_of_locs+nr_of_objs+1) {
    sprintf(outputline, "BuildCList(): there migt be a loop in the containment tree (list size exceeded).\n");
    Output(outputline);
    return(ERROR);
  }
  ...
  switch (value) {
    ...
    case 1:
      *word = *word | (1 << bit_pos);
      break;
    default:
      sprintf(outputline, "SetBitVal(): error in value, must be 1 or 0.\n");
      Output(outputline);
      break;
  }
}
 ...
 int32_t par;
{
  ...
  while (par != NO_ID && i < (nr_of_locs+nr_of_objs) && result != DISAGREE && result != QUIT) {
    next_result = XeqTrigger(par, EXITT);
 ...
 int32_t par;
{
  ...
  while (par != NO_ID && i < nr_of_locs+nr_of_objs && result != DISAGREE && result != QUIT) {
    next_result = XeqTrigger(par, ENTR);
    if (next_result != AGREE && next_result != NO_MATCH)
      result = next_result;
    par = list[++i];
  }
 ...
 int32_t par;
{
  ...
  while (end_result == AGREE && i<objs.nr_of_objects) {
    ...
    if ((result = Entrance(objs.object_ids[i])) != AGREE)
      end_result = result;
    i++;
  }
 ...
 int32_t dest;
{
  ...
  if (contained_objs->nr_of_objects >= MAX_CONTAINED_OBJECTS) {
    sprintf(outputline, "Move: Internal error, no more room in dest (%d)\n",
  ...
  if (i == contained_objs->nr_of_objects) {
    sprintf(outputline, "Move(): internal error. %d %d\n", i, contained_objs->nr_of_objects);
    Output(outputline);
    return(ERROR);
  }
 ...
 int32_t id;
{
  ...
  if (IsLocId(id)) {
    ...
    if (IsObjId(id)) {
      ...
      sprintf(outputline, "Shuffle(): Id must be location or object (%d).", id);
      Output(outputline);
      return(ERROR);
    }
  }
...
int32_t Synchronize(id, trigger_id, flag, flag_val, level)
 int32_t id;
 int32_t trigger_id;
 int32_t flag;
 int32_t flag_val;
 int32_t level;
{
  ...
  while (i<index && cont) {
    if (TestCFlag(list[i], flag) == flag_val) {
      ...
      result = XeqTrigger(list[i], trigger_id);
      switch (result) {
        case AGREE:
          count++;
          break;
        ...
        case NO_MATCH:
          ...
          break;
        default:
          sprintf(outputline, "Synchronize(): unexpected return value %d from XeqTrigger.\n", result);
          Output(outputline);
          break;
      }
    }
 ...
 int32_t value;
{
  if (sp == STACK_SIZE) {
    ...
    sprintf(outputline, "INTERNAL ERROR: STACK OVERFLOW.\n");
    Output(outputline);
  }
  else
    stack[sp++] = value;
...
int32_t Pop()
{
  if (sp == 0) {
    /* Stack is empty. */
    sprintf(outputline, "INTERNAL ERROR: STACK UNDERFLOW.\n");
    Output(outputline);
