...
/* that the user may use in his sourcefile.                   */
/**************************************************************/
  ...
  if (CheckPars(operation, type1, type2, NO_TYPE, NO_TYPE, NO_TYPE)) {
    ...
    switch (operation) {
      ...
      case GREATER_THAN:
        ...
        break;
      default:
        sprintf(outputline, "XeqLtGt(): unknown operation %d\n", operation);
        Output(outputline);
        return(ERROR);
    } /* switch */
  } /* if */
  ...
  int32_t **trigger;
{
  ...
  if (par2 == NONE) {
    ...
    if (type1 == COMMON_ATTRS || type1 == ATTRIBUTES) {
      ...
      if (type1 == TIM_ID) {
        ...
        /* serious trouble */
        printf("XeqDiv() fatal error: attribute or timer expected.\n");
        return(ERROR);
      }
    }
  }
  ...
  switch (opr) {
    ...
    case QUOT:
      if (!CheckPars(QUOT, type1, type2, type3, NO_TYPE, NO_TYPE))
        return(QUIT);
      if (par3 == 0) {
        printf("XeqBasicOperator(): division by zero.\n");
        return(QUIT);
      }
    ...
    case REM:
      ...
      break;
    default:
      sprintf(outputline, "XeqBasicOperator(): illegal operator %d\n", opr);
      Output(outputline);
      return(ERROR);
  } /* switch */
  ...
  if (type1 == COMMON_ATTRS || type1 == ATTRIBUTES) {
    ...
    if (type1 == TIM_ID)
      ...
      /* serious trouble */
      printf("XeqBasicOperator() fatal error: attribute or timer expected.\n");
      return(ERROR);
    }
  }
  return(CONTINUE);
