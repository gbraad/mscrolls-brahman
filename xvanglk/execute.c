...
int32_t  EvalCond(int32_t**, int32_t*);
int32_t  SkipCond(int32_t**);
int32_t  Ite(int32_t**, int32_t, int32_t*);
int32_t  MatchActionRec(compActionRec*, usrActionRec*, int32_t);
int32_t  XeqActionRec(usrActionRec*, int32_t*, int32_t);
int32_t  XeqPrologue(int32_t);
int32_t  XeqEpilogue(int32_t);
int32_t  XeqVerbDefault(usrActionRec*, int32_t);
int32_t  Execute(int32_t*);
int32_t  XeqTrigger(int32_t, int32_t);
 ...
 int32_t id;
{
  ...
  if (!(IsLocId(owner) || IsObjId(owner))) {
    sprintf(outputline, "GetDesc(): internal error: wrong id type %d.\n", owner);
    Output(outputline);
    return(NULL);
  }
  ...
  if (dp == NULL) {
    ...
    /* return NULL pointer.                       */
    if (IsLDescrId(id)) {
      sprintf(outputline, "GetDescr(): internal error: local description not found (%d, %d).\n", owner, id);
      Output(outputline);
    }
    return(NULL);
  }
 ...
 int32_t *triggers[];
{
  ...
  if (tp != NULL)
    ...
    if (IsLTriggId(id)) {
      sprintf(outputline, "GetTrigger(): internal error: local trigger not found.\n");
      Output(outputline);
      return(ERROR);
    }
  }
...
int32_t EvalCond(trigger, opcode)
 int32_t **trigger;
 int32_t *opcode;
{
    ...
    switch (*opcode) {
      ...
      default:
        ...
        if (IsTestFun(*opcode) || IsCAttrId(*opcode) ||
                                  IsLAttrId(*opcode)) {
          eval_arr[eval_ptr++] = XeqTestFun(*opcode, trigger);
          break;
        }
        else {
          sprintf(outputline, "EvalCond(): unexpected opcode: %d.\n",*opcode);
          Output(outputline);
          return(ERROR);
        } /* else */
    } /* switch */
    ...
    switch (opcode) {
      ...
      default:
        if (IsTestFun(opcode) || IsCAttrId(opcode) || IsLAttrId(opcode))
          ...
          sprintf(outputline, "SkipCond(): unexpected opcode: %d.\n", opcode);
          Output(outputline);
          return(ERROR);
        }
        break;
...
int32_t Ite(trigger, state, opcode)
 int32_t **trigger;
 int32_t state;
 int32_t *opcode;
{
  int32_t new_state  = XEQ;
  int32_t result     = CONTINUE;
    ...
    switch (*opcode) {
      ...
      case IF:
        switch (state) {
          ...
          case SKIP:
            ...
            break;
          default:
            sprintf(outputline, "The impossible just happened\n");
            Output(outputline);
            return(ERROR);
        } /* switch */

        result = Ite(trigger, new_state, opcode);
        if (result != CONTINUE)
      ...
      case ELSE:
        if (first_else) {
          ...
          *opcode = NextOpcode(trigger);
          ...
          return(CONTINUE);  /* Don't read new opcode. */}
      break;
      case ENDIF:
        *opcode = NextOpcode(trigger);
        return(result);
      default:
        ...
        if (!(IsIntAct(*opcode) || IsCAttrId(*opcode) ||
                                   IsLAttrId(*opcode))) {
          sprintf(outputline, "Ite(): bad opcode %d.\n", *opcode);
          Output(outputline);
        ...
        switch (state) {
          ...
          case XEQ:
            result = XeqIntAct(*opcode, trigger);
            switch (*opcode) {
                         ...
                         /* user input.                         */
{
  ...
  if (source->action != target->action &&
      (source->action != ACTION || target->action == NO_ID))
    return(ERROR);
...
int32_t XeqActionRec(action_rec, xeq_list, subject_index)
                             ...
                             /* action_rec.                     */
{
  ...
  id = xeq_list[0];
  while (id != NO_ID && i < (nr_of_locs+nr_of_objs) && result != GET_SUBJECT &&
        result != GET_SPECIFIER && result != GET_ANSWER &&
        result != DISAGREE && result != QUIT) {
    ...
    if (IsLocId(id))
      ...
      sprintf(outputline, "XeqActionRec(): illegal identifier. Location or object was expected\n");
      Output(outputline);
      return(DISAGREE);
    }
    ...
    while (arec != NULL && !found) {
      if (MatchActionRec(arec, action_rec, subject_index)) {
        ...
        next_result = XeqTrigger(arec->execute[0], arec->execute[1]);
        if (next_result != NO_MATCH)
          result = next_result;
      }
      else
        arec = arec->next;
    ...
    if (!found) {
      next_result = XeqTrigger(id, DEFAULT_TRIGG);
        if (next_result != NO_MATCH)
          result = next_result;
    }
 ...
 int32_t verb_id;
{
  ...
  if (!InMem(verb_id))
    switch (Load(verb_id)) {
      case ERROR:
        sprintf(outputline, "XeqPrologue(): internal error.\n");
        Output(outputline);
        return(QUIT);
      case NO_MATCH:
        return(NO_MATCH);
  ...
  if ((verb->action_rec)->action == PROLOGUE)
    return(Execute(verb->code));

  return(NO_MATCH);
}


int32_t XeqEpilogue(verb_id)
 int32_t verb_id;
{
  ...
  if (!InMem(verb_id))
    switch (Load(verb_id)) {
      case ERROR:
        sprintf(outputline, "XeqEpilogue(): internal error.\n");
        Output(outputline);
        return(QUIT);
      case NO_MATCH:
        return(NO_MATCH);
  ...
  if ((verb->action_rec)->action == EPILOGUE)
    code = verb->code;
  else if (verb->next != NULL &&
           verb->next->action_rec->action == EPILOGUE)
    code = verb->next->code;
  else
    /* No epilogue. */
    return(NO_MATCH);
...
int32_t XeqVerbDefault(action_rec, subject_index)
 usrActionRec *action_rec;
 int32_t      subject_index;
{
  ...
  if (!InMem(action_rec->action))
    switch (Load(action_rec->action)) {
      case ERROR:
        sprintf(outputline, "XeqVerbDefault(): internal error.\n");
        Output(outputline);
        return(QUIT);
 ...
 int32_t *trigger;
{
  ...
  int32_t new_state = XEQ;
  int32_t opcode;
  int32_t result    = AGREE; /* By default, a trigger returns agree. */
  ...
  while (1) {
    switch (opcode) {
      case END_OF_CODE:
        return((result == CONTINUE? AGREE : result));
      case IF:
        switch (state) {
          case XEQ:
            new_state = XEQ;
        ...
        if ((result = Ite(&trigger, new_state, &opcode)) != CONTINUE)
          /* Finished with this trigger. */
          return(result);
        break;
      default:
        ...
        if (!(IsIntAct(opcode) || IsCAttrId(opcode) ||
                                  IsLAttrId(opcode))) {
          sprintf(outputline, "Execute(): bad opcode %d.\n", opcode);
          Output(outputline);
          return(QUIT);
        }
