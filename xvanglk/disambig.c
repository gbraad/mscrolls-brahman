 ...
 match       *specifier_hits;
{
  ...
  if ((start = (parserActionRec*) malloc(sizeof(parserActionRec))) == NULL) {
    PrintString("ApplyParserRules(): error in malloc().\n");
    return(NULL);
  }
  ...
  for (i=0; i<total_recs; i++) {
    if ((current->next = (parserActionRec*) malloc(sizeof(parserActionRec))) == NULL) {
      PrintString("CreateParserRecs(): error in Malloc().\n");
 ...
 parserActionRec *parser; /* looking for this one */
{
  ...
  if (verb->action != parser->action &&
      (verb->action != ACTION || parser->action == NO_ID))
    return(ERROR);
 ...
 parserActionRec *parser_recs;
{
  ...
  if (!InMem(parser_recs->action))
    switch (Load(parser_recs->action)) {
      case ERROR:
        PrintString("FindParserRules(): internal error.\n");
        return(NULL);
  ...
  verb = verbs[(parser_recs->action)-FIRST_VERB_ID];
  ...
  while (cont && verb != NULL) {
    ...
    while (cont && arec != NULL) {
      if (arec->action == DEFAULT || MatchParserRec(arec, parser_recs)) {
...
/*PrintParserActionRec(parser_recs);*/
